// Enhanced Supabase Service Implementation
...
class SupabaseService extends BaseService {
  ...
  // Cruise Methods
  async getCruises(filters?: any): Promise<ApiResponse<Cruise[]>> {
    this.ensureInitialized();
    
    try {
      let query = this.client!.from('cruises').select('*');
      ...
      const cruises: Cruise[] = data.map(item => ({
        id: item.id,
        name: item.name,
        image: item.image,
      ...
      const appError = AppErrorHandler.handleDatabaseError(error);
      AppErrorHandler.logError(appError, 'SupabaseService.getCruises');
  ...
  async getCruiseById(id: string): Promise<ApiResponse<Cruise | null>> {
    this.ensureInitialized();
  ...
  async getHotels(filters?: any): Promise<ApiResponse<Hotel[]>> {
    ...
    try {
      let query = this.client!.from('hotels').select('*');
      ...
      const appError = AppErrorHandler.handleDatabaseError(error);
      AppErrorHandler.logError(appError, 'SupabaseService.getHotels');
  ...
  async updateBookingStatus(bookingId: string, status: Booking['status']): Promise<ApiResponse<Booking>> {
    ...
    try {
      ...
      const appError = AppErrorHandler.handleDatabaseError(error);
      AppErrorHandler.logError(appError, 'SupabaseService.updateBookingStatus');
  ...
  subscribeToBookings(userId: string, callback: (booking: Booking) => void) {
    this.ensureInitialized();
        ...
        (payload) => {
          if (payload.new) {
            const booking: Booking = {
              ...
              travelDate: payload.new.travel_date,
              status: payload.new.status,
              totalAmount: payload.new.total_amount,
              commissionAmount: payload.new.commission_amount,
